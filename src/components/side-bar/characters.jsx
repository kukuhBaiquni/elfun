...
function Characters() {
  const TextColor = (def) => (def ? 'group-hover:text-gray-500' : 'group-hover:text-gray-200')
  return (
    <Fragment>
      Characters ({character.length})
      <ul className='my-2'>
        {character.map((item, index) => (
          <li className={`relative z-10 whitespace-nowrap group font-normal ${item.hoverBgColor} hover:translate-x-2 rounded cursor-pointer transition-transform duration-300`} key={item.name}>
